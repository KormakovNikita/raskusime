...
**До одобления сайта** держите `YANDEX_RSYA_ENABLED=false` — рекламные блоки на странице не должны показываться до модерации.

**Важно:** реклама **не показывается** во время показа записки (только в SEO-зоне главной). Блоки не размещайте рядом с кнопкой «Раскусить».

### Если сайт отклонён («по иным причинам»)

Типичные причины для сервиса вроде Раскусi:

| Причина | Что сделать |
|--------|-------------|
| Одностраничник (п. 1.1.k) | На сайте есть `/about.html`, `/contacts.html`, `/articles/` — убедитесь, что они открываются и проиндексированы |
| Мало контента / «сайт под рекламу» | Статьи в `/articles/` — редакционный контент, не только генератор |
| Пустой `ads.txt` | В `.env` на VPS задайте реальную строку из кабинета Partner |
| Мало трафика (п. 1.1.x) | Подключите Директ или органику, дождитесь стабильных визитов |
| Повторная подача | После отказа «по иным причинам» — **не раньше чем через 30 дней** |

Перед повторной отправкой проверьте:

- [ ] `https://raskusime.ru/about.html`, `/contacts.html`, `/articles/` открываются
- [ ] `https://raskusime.ru/sitemap.xml` содержит все страницы
- [ ] `https://raskusime.ru/ads.txt` — рабочая строка, не комментарий-заглушка
- [ ] На сайте **нет** рекламных блоков до одобрения (`YANDEX_RSYA_ENABLED=false`)
- [ ] В кабинете Partner → «Отправить на модерацию» после деплоя

Если уверены, что всё исправлено, но отказ повторяется — напишите в поддержку Partner с URL и описанием изменений.
